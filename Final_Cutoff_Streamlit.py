    ...
    with tab1:
        ...
        if st.button("Autodetect"):
            if session_state.files is not None:
                ...
                porosity_options = ['PIGE_QEPP', 'PIGE', 'PHIE_F', 'PHIE', 'PIGE_F','PHIE_Final','PHIE_FINAL','PIGE_RA','PIGN_QEPP']
                saturation_options = ['SUWI', 'SW_F', 'SW', 'SUWI_QEPP', 'sw', 'Sw_F','SWE','SWE_Final','SWE_FINAL','SUWI_RA','SUWI_QEPP']
                vcl_options = ['VCL', 'VCL_F', 'VCL_QEPP', 'VCL_GEO_QEPP','VSH','VSH_Final','VSH_FINAL','VCL_RA','Shale_QEPP']
...
def page5():
    ...
    with tab3:
        ...
        if segment_saturation and len(segment_saturation)==len(session_state.dict_Rawdataframe):
            ...
            # Update y-axis with range based on df_p['MD'] and reverse the y-axis
            fig3.update_yaxes(nticks=20, range=[df_p['MD'].max(), df_p['MD'].min()], showgrid=True)
            fig3.add_hline(y=y_value_OWC, line=dict(color="red", width=2, dash="dashdot"),annotation_text="OWC/OB", annotation_position="bottom right",annotation_font_size=20,annotation_font_color="red")
            fig3.add_hline(y=y_value_GOC, line=dict(color="green", width=2, dash="dashdot"),annotation_text="GOC/GB", annotation_position="bottom right",annotation_font_size=20,annotation_font_color="green")
            # Show plot
            st.plotly_chart(fig3)
